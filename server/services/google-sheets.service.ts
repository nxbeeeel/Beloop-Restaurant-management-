...
export class GoogleSheetsService {
    ...
    static async saveUrl(prisma: PrismaClient, outletId: string, url: string) {
    ...
    static async getUrl(prisma: PrismaClient, outletId: string) {
        const outlet = await prisma.outlet.findUnique({
            where: { id: outletId },
            select: { googleSheetsUrl: true }
        });
        return outlet?.googleSheetsUrl || null;
    }

    static async exportMonthlyReport(prisma: PrismaClient, params: {
        outletId: string;
        tenantId: string | null | undefined;
        year: number;
        month: number;
    }) {
        const { outletId, tenantId, year, month } = params;
        ...
        const salesData = sales.map(sale => ({
            ...
            submittedBy: sale.staff?.name || 'Unknown',
        }));
        ...
        const expensesData = expenses.map(expense => ({
            ...
            submittedBy: expense.staff?.name || 'Unknown',
        }));
    ...
    static async generateScript(prisma: PrismaClient, params: {
        outletId: string;
        tenantId: string | null | undefined;
    }) {
        const { outletId, tenantId } = params;
